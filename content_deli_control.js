...
function content_deli_control()
{
    ...
    if ($session.opened_qz_ord !== null) // draw picked quiz
    {
        ...
        $res += "<div class='subhead'>" +
                    ...
                    "<span class='qz_name'>" + qz.name + "</span>" +
                    "<div class='date'><span class='label'>дата окончания</span> " +timestamp_from_date(qz.settings.end_date, true)+ "</div>" +
                "</div>";
    ...
    else // draw selection ROSTER
    {
        ...
        else
        {
            ...
            active_qz_ord_list.forEach(function (v_qz_ord, list_ord) {
                ...
                {
                    let qz_name = "";
                    if ($ad.qzs[v_qz_ord] && $ad.qzs[v_qz_ord].hasOwnProperty("name"))
                        qz_name = $ad.qzs[v_qz_ord].name;
