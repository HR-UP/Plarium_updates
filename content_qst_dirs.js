    ...
    */
    ...
    if (action === "add_events")
    {
        $(".dirs_wnd .icon")
            .off("mousedown")
            .mousedown(function () {
                if ("qst" === $nav.screen) // forbid to do this in deli screen when picking a qbook for the focus group
                {
                    ...
                    if ($(this).attr("type") === "qb" && is_qb)
                    {
                        $curs.drag = true;
                        $curs.qb_id = id;
                        let qb_ord = get_qb_ord_from_qb_id($curs.qb_id);
                        $curs.lay_name = $ad.qbooks[qb_ord].meta.lay_name;
                        $curs.lay_id = $ad.qbooks[qb_ord].lay_id;
                        $(this)
                            .css("transform","rotate(15deg)")
                            .css("filter","grayscale(0%)");

                        // show "input to this dir" icons to all valid locations
                        ...
                        // show "swap with qbook index" icons to all valid locations
                        $(".dirs_wnd .icon").each(function () {
                            let is_qb = $(this).closest(".line").attr("is_qb") * 1;
                            if (is_qb)
                            {
                                let targ_qb_id = $(this).closest(".line").attr("uid") * 1;
                                let targ = {};
                                targ.ord = get_qb_ord_from_qb_id(targ_qb_id);
                                targ.lay_name = $ad.qbooks[targ.ord].meta.lay_name;
                                targ.lay_id = $ad.qbooks[targ.ord].lay_id;

                                if ($curs.lay_name === targ.lay_name &&
                                    $curs.lay_id === targ.lay_id &&
                                    targ_qb_id !== id) // swap avaliable only inside same folder
                                {
                                    //let layer = dirs_get_layer_relative_tag("host", $(this).attr("type"));
                                    let s = "<div class='qb_insert' uid='"+ targ_qb_id +"'></div>";
                                    $(this).append(s);
                                }
                            }
                        ...
                        $(".qb_insert")
                            .mouseenter(function () {
                                ...
                                {
                                    $(".qb_insert").css("filter","grayscale(100%)"); // gray all
                                    $(this).css("filter","grayscale(0%)"); // saturate self

                                    $curs.swap_qbook = {
                                        id: $(this).attr("uid") * 1
                                    };
                                    console.log("$curs.swap_qbook", $curs.swap_qbook);
                                }
                            })
                            .mouseleave(function () {
                                if ($curs.drag && $curs.hasOwnProperty("qb_id"))
                                {
                                    ...
                                    if ($curs.hasOwnProperty("swap_qbook"))
                        ...
                        $(".dir_insert")
                            .mouseenter(function () {
                                if ($curs.drag && $curs.hasOwnProperty("qb_id"))
                                {
                                    ...
                                    //console.log("$curs.insert_folder", $curs.insert_folder);
                                }
                            })
                            .mouseleave(function () {
                                if ($curs.drag && $curs.hasOwnProperty("qb_id"))
                ...
                $(".dirs_wnd .line_qb").each(function () {
                    ...
                    if($ad.qbooks.length)
                        for(let i=0; i<$ad.qbooks.length; i++)
                            if ($ad.qbooks[i].id === qb_id &&
                                $ad.qbooks[i].lay_id === host_id &&
                                $ad.qbooks[i].meta.lay_name === type)
                            {
        ...
        $(".toolbox .qb_save")
            ...
            .click(function(){
                ...
                form.meta = {};
                form.meta.lay_name = form.lay_name;
                form.meta.lay_ind = dirs_get_latest_layer_index(form.lay_name) + 1;
        ...
        $(".toolbox .qb_del")
            ...
            {
                floater_hint("show",{
                    head: "Удалить опросник",
                    tx: "Нажмите чтобы полностью удалить данный опросник.",
                    w: 400,
                    dx: -200,
                    dy: 20,
                });
            ...
            .click(function(){
                ...
                form.qb_name = $ad.qbooks[form.qb_ord].name;

                // QUIZES LIST
                form.qz_list = [];
                // detect all the affected quizes
                $ad.qzs.forEach(function (v_qz, i_qz) {
                    v_qz.settings.comm_groups.forEach(function (v_gr, i_gr) {
                        if (v_gr.qb_id === form.qb_id)
                        {
                            let slot = {
                                qz_id: v_qz.id
                            };
                            slot.qz_ord = i_qz;
                            slot.qz_name = v_qz.name;
                            slot.gr_ord = i_gr;

                            let resp_id = v_qz.resps[i_gr][0].id;
                            slot.resp_ord = get_resp_ord_from_resp_id(resp_id);
                            slot.focus_name = $ad.resps[slot.resp_ord].fio;

                            form.qz_list.push(slot);
                        }
                    });
                });

                message_ex("show","confirm","qbook_del", form);
            });
        ...
        $(".toolbox .qb_duplicate")
            ...
            .click(function(){
                let form = {};
                form.lay_name = $(this).closest(".header").find(".icon").attr("line"); // $(this).closest(".line").find(".icon").attr("line") ;//+ "_list";
                form.lay_id = $(this).closest(".line").attr("uid") * 1;
...
function dirs_siblings_exist(layer, id) {
    ...
    if ($ad.qbooks.length)
        for (let i=0; i<$ad.qbooks.length; i++)
            if ($ad.qbooks[i].meta.lay_name === layer && $ad.qbooks[i].lay_id === id)
            {
                found = true;
                break;
            }
...
function dirs_del_layer(layer, id, mode) {
    ...
    if ($ad.qbooks.length)
        $ad.qbooks.forEach(function (v_qb) {
            if (v_qb.meta.lay_name === layer && v_qb.lay_id === id)
            {
                let last_index = dirs_get_latest_layer_index(null) + 1;
                // Aplly changes to qbook right away so the _dirs_get_latest_layer_index_ would detect last index correctly
                let qb_ord = get_qb_ord_from_qb_id(v_qb.id);
                $ad.qbooks[qb_ord].meta.lay_name = null;
                $ad.qbooks[qb_ord].meta.lay_ind = last_index;
                $ad.qbooks[qb_ord].meta.lay_id = 0;
...
//----------------------------------------------------------------------------------------------------------------------
function dirs_get_latest_layer_index(lay_name) {
    let latest = 0;
    $ad.qbooks.forEach(function (v_qb) {
        if (v_qb.meta.lay_ind > latest)
            latest = v_qb.meta.lay_ind;
    });
    return latest;
}

//----------------------------------------------------------------------------------------------------------------------
function dirs_bld_visible_qbooks_list() {
    $pers.qbooks_vis_list = [];
    dirs_get_visible_qbook_ids("di", null);
    return $pers.qbooks_vis_list;
}
//----------------------------------------------------------------------------------------------------------------------
function dirs_get_visible_qbook_ids(layer, id) {

    function get_related_qb_ids(layer, host_id) {
        if ($ad.qbooks.length)
            $ad.qbooks.forEach(function (v_qb) {
                if ((v_qb.meta.lay_name === layer || (layer === null && !v_qb.meta.lay_name)) &&
                    v_qb.lay_id === host_id)
                {
                    if ($pers.qbooks_vis_list.indexOf(v_qb.id) === -1)
                        $pers.qbooks_vis_list.push(v_qb.id);
                }
            });
    }
        ...
        if (src.length) // Build lines of this layer level
            src.forEach(function (v_slot) {
                if (null === id || id === v_slot.host_id) // Only if this is a sub-layer for our upper-layer or root
                    ...
                    {
                        let self_id = v_slot.id;
                        get_related_qb_ids(layer, self_id); // Bld all related qbooks

                        let child_layer = dirs_get_layer_relative_tag("child", layer); // There is a sub-layer based on a fixed structure
                        dirs_get_visible_qbook_ids(child_layer, self_id);
                    }
            })
    }
}
...
function bld_related_qb_lines(layer, host_id) {
    ...
    let lines = []; // save each qbook line by its index inside the folder
    if ($ad.qbooks.length)
        $ad.qbooks.forEach(function (v_qb) {
            // #TEST
                ...
                v_qb.lay_id === host_id)
            {
                let line = "";
                let mark = "";
                if (!v_qb.meta.lay_name)
                    mark = "toplvl";

                line += "<div class='line line_qb' uid='"+ v_qb.id +"' "+mark+" is_qb='1'>";
                line += "<div class='header'>";
                line += "<div class='icon' type='qb' line='qb'></div>";

                // Add adaptaion btn icon (only when in relevant screen)
                if (4 === $new_qz.qst_screen_id)
                    line += "<div class='qb_adapt'></div>";
                ...
                line += "<span class='name' is_adapted='"+ is_adapted +"' show_panel='0'>" + v_qb.name + "</span>";
                line += qb_toolbox ;
                line += "</div>";
                line += "</div>";

                lines[v_qb.meta.lay_ind] = line;
            }
        });

    // Fill qbooks line by line maintaining indexed order
    if (lines.length)
        lines.forEach(function (v_line) {
            if (v_line)
                s += v_line;
        });
