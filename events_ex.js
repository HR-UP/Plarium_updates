...
// $data — custom data to show/calculate
    ...
    function set_content($action,$type,$about,$data,$tags) {
        ...
        switch ($about) {
            ...
            case "qbook_del":
                ...
                $head = "Полное удаление опросника \""+ $ad.qbooks[$data.qb_ord].name +"\"";
                if (!$data.qz_list.length)
                    $cont = "Данный опросник не задействован в опросах.";
                else
                {
                    $cont = "Данный опросник задействован в таких опросах:<br><br>";
                    $data.qz_list.forEach(function (v_slot, i) {
                        if (i)
                            $cont += "<br>";

                        $cont += (i+1) + ") опрос <b>" + v_slot.qz_name + "</b> фокус-группа <b>" + v_slot.focus_name + "</b>";
                    });
                }
                break;
            ...
            case "save_as_template":
                $head = "Сохранение шаблона ответов";
                $cont = "Введите название для нового шаблона ответов.";
                break;

            case "an_folder":
                $head = "Добавить папку аналитики";
                $cont = "Введите название для новой папки ниже.";
                break;

            case "add_group_to_deli_list":
                $head = "Добавить группу респондентов";
                $cont = "Вы действительно хотите добавить группу респондентов к текущему опросу?<br><br><b>Будут добавлены:</b><br>";
                let group = $ad.qzs[$data.qz_ord].resps[$data.gr_ord];
                group.forEach(function (v_resp, i_resp) {
                    let fio = get_resp_ord_from_resp_id(v_resp.id);
                    fio = $ad.resps[fio].fio;
                    $cont += (i_resp + 1) + ") " + fio +" (<i style='font-size: 12px;'>"+ g_cats_name_list_short[v_resp.cat_id] +"</i>)<br>";
                });
                break;

            case "add_whole_qz_to_list":
                $head = "Добавить группу респондентов";
                $cont = "Вы действительно хотите добавить все группы респондентов из опроса <b>"+ $ad.qzs[$data.qz_ord].name +"</b> к текущему опросу?<br><br>" +
                    "<b>Будут добавлены фокус-группы:</b><br>";
                let resps = $ad.qzs[$data.qz_ord].resps;
                resps.forEach(function (v_gr, i_gr) {
                    let focus_fio = get_resp_ord_from_resp_id(v_gr[0].id);
                    focus_fio = $ad.resps[focus_fio].fio;
                    $cont += (i_gr + 1) + ") " + focus_fio +" (<i style='font-size: 12px;'>"+ v_gr.length +" участников</i>)<br>";
                });
                break;

            case "qz_reschedule":
                let qz = $ad.qzs[$data.qz_ord];
                let cur_enddate = timestamp_from_date(qz.settings.end_date, true);
                let action_tx = "Ниже Вы можете указть новую дату звершения опроса.<br> ";
                action_tx += "Текущая дта завершения: <b>"+ cur_enddate +"</b>";

                $head = "Переназначить дату завершения опроса <b>" + qz.name + "</b>";
                $cont = action_tx;
                break;
    ...
    function set_structure($action,$type,$about,$data,$tags) {
        ...
        switch ($type) {
            ...
            case "target_qb_select":
            {
                ...
                $res += "<div class='btn_box'>";
                    $res += "<div action='yes' class='btn'>Ок</div>";
                $res += "</div>";
                break;
            }
            ...
            case "memo":
            {
                ...
                // Блок текстового ввода
                $res += "<div class='inp_box'>";
                $res += "<textarea style='text-align: left; min-width: 300px; height: 60px;' type='text' class='inp' placeholder='По-желанию введите уточняющий текст' value='"+insert_value+"'></textarea>";
                $res += "</div>";
                $res += "<div class='btn_box'>";
                $res += "<div action='yes' valid='1' class='btn pos_rght'>Подтвердить</div>";
                $res += "<div action='no' class='btn pos_left'>Отмена</div>";
            ...
            case "qz_reschedule":
            {
                ...
                // Блок текстового ввода
                $res += "<div class='inp_box'>";
                $res += "<input type='date' class='end_date'>";
                $res += "</div>";
                $res += "<div class='btn_box'>";
                $res += "<div action='yes' valid='1' class='btn pos_rght'>Подтвердить</div>";
                $res += "<div action='no' class='btn pos_left'>Отмена</div>";
    ...
    function set_events($action,$type,$about,$data,$tags) {
        ...
        switch ($about)
        {
                    ...
                    .click(function(){
                        ...
                        if (is_qb)
                        {
                            ...
                            else
                            {
                                ...
                                $new_qz.settings.comm_groups[ord].last_qb_id = qb_id;
            ...
            case "qz_reschedule":
            {
                let e_date = $ad.qzs[$data.qz_ord].settings.end_date;
                let d = new Date();
                d.setTime(e_date * 1000);
                ...
                if (day.length === 1)
                    day = "0" + day;
                let v = d.getFullYear().toString() +"-"+ mon + "-"+ day;
                $(".inp_box .end_date").val(v); // set the current edn_date on the calendar
            }
        }
    }
                            ...
                            if ($valid === 1)
                            {
                                switch ($about)
                                {
                                    ...
                                    case "resp_delete_batch_from_list":
                                        ...
                                        $new_qz.settings.comm_groups.splice($data, 1);
                                    ...
                                    case "qbook_save_as":
                                        ...
                                        qbook_names_list = [];
                                        dirs_bld_visible_qbooks_list(); // refreshes the $pers.qbooks_vis_list

                                        if ($ad.qbooks.length)
                                            $ad.qbooks.forEach(function (v_qb) {
                                                if ($pers.qbooks_vis_list.indexOf(v_qb.id) !== -1) // check along only with visible qbooks
                                        ...
                                        if ($ad.qbooks.length && qbook_name)
                                            $ad.qbooks.forEach(function (v_qb, i_qb) {
                                                if (i_qb !== $data.qb_ord && // exclude own qb_name from the list of match-search
                                                    $pers.qbooks_vis_list.indexOf(v_qb.id) !== -1 // check along only with visible qbooks
                                                )
                                                    qbook_names_list.push(v_qb.name);
                                            });
                                    ...
                                    case "qb_duplicate":
                                        ...
                                        else
                                        {
                                            ...
                                            Object.keys($data).map(function (key, ind) {
                                                ...
                                                {
                                                    form[key] = $data[key];
                                                    form[key].lay_name = qb_src.lay_name;
                                                }
                                    ...
                                    case "qbook_del":
                                        ...
                                        sendAJ("qbook_del", JSON.stringify($data)); // update all affected qbook relation tags
                                        break;

                                    case "dir_delete":
                                        dirs_del_layer($data.lay_name, $data.lay_id, "remove"); // get all deleted in list: folder and sub-folders, and qbooks with unsigned to the root
                                        //console.log("dirs delete, final_list", final_list);
                                        sendAJ("qbook_dirs_update", JSON.stringify($data.final_list.qbooks)); // update all affected qbook relation tags
                                        form = {
                                            action: "save",
                                            dirs: $pers.dirs
                                        };
                                    ...
                                    case "group_resp_add":
                                        ...
                                        if (2 === form.cat_id)
                                            form.cat_id = 3; // employee
                                        else
                                            form.cat_id = 1; // bro team
                                    ...
                                    case "save_as_template":
                                    {
                                        form = {
                                            list: $data,
                                            name: $(".sys_mes .inp_box .inp").val()
                                        };

                                        if (!form.name)
                                        {
                                            block_autoclose = true;
                                            message_ex("show","info","direct","Введите корректное название/имя шаблона.");
                                        }
                                        else
                                            sendAJ("save_as_template", JSON.stringify(form));
                                        break;
                                    }
                                    ...
                                    case "an_folder":
                                    {
                                        ...
                                        {
                                            block_autoclose = true;
                                            message_ex("show","info","direct","Введите корректное название папки.");
                                        }
                                        else
                                        {
                                            let new_folder = {
                                            ...
                                            if (!$pers.hasOwnProperty("an_folders"))
                                                $pers.an_folders = [];
                                            $pers.an_folders.push(new_folder);

                                            sendAJ("save_an_folder", JSON.stringify($pers.an_folders));
                                        }
                                    ...
                                    case "add_group_to_deli_list":
                                    {
                                        block_autoclose = true;
                                        message_ex("hide");
                                        let response = content_deli_qp_box("add_group", $data);// {gr_ord:int, qz_ord:int}
                                        if (response)
                                        {
                                            build_deli_list_table(false); // update deli list
                                            content_deli_events();
                                            $session.set();
                                            message_ex("show","info","direct","Группа успешно добавлена");
                                        }
                                        break;
                                    }
                                    ...
                                    case "add_whole_qz_to_list":
                                    {
                                        block_autoclose = true;
                                        message_ex("hide");
                                        let response = content_deli_qp_box("add_quiz", $data);// {gr_ord:int, qz_ord:int}
                                        if (response)
                                        {
                                            build_deli_list_table(false); // update deli list
                                            content_deli_events();
                                            $session.set();
                                            message_ex("show","info","direct","Опрос успешно добавлен<br>Было добавлено <b>"+ response +"</b> групп.");
                                    ...
                                    case "qz_reschedule":
                                        let new_end_date = $(".inp_box .end_date").val();
                                        let d = new_end_date.split("-");
                                        let dx = [d[2], d[1], d[0]];
                                        new_end_date = timestamp_from_date(dx);
                                        console.log("new_end_date: " + new_end_date);
                                        form = {
                                            end_date: new_end_date,
                                            qz_id: $ad.qzs[$data.qz_ord].id
                                        };
                                        sendAJ("qz_new_end_date", JSON.stringify(form));
                                        break;
                                }
                                ...
                                switch ($tags)
                                {
                                    ...
                                    case "qz_rename":
                                        ...
                                        form.qz_ord = $data.qz_ord;
                                        form.qz_id = $ad.qzs[form.qz_ord].id;
                                        form.qz_name = $(".sys_mes .inp_box .inp").val();

                                        if (form.qz_name)
                                            sendAJ("qz_rename", JSON.stringify(form));
                                        else
                                        {
                                            block_autoclose = true;
                                            message_ex("show","info","direct","Пожалуйста, введите валидное название опроса");
                                        }
                                    ...
                                    case "qz_mute":
                                        let is_muted_new = Math.abs($ad.qzs[$session.opened_qz_ord].settings.is_muted - 1);
                                        $ad.qzs[$session.opened_qz_ord].settings.is_muted = is_muted_new;


                                        if (is_muted_new)
                                        {
                                            $(".qz_name").append(muted_sign); // add sign text
                                            let tx_desc = $(".sys_mes .inp_box .inp").val().trim();
                                            tx_desc = tx_desc.replace(/"/g,"\"");
                                            tx_desc = tx_desc.replace(/;/g,"");
                                            tx_desc = tx_desc.replace(/\n/g,"<br>");
                                            $ad.qzs[$session.opened_qz_ord].settings.is_muted_tx = tx_desc;
                                        }
                                        else
                                        {
                                            $(".muted_sign").remove();
                                            $ad.qzs[$session.opened_qz_ord].settings.is_muted_tx = "";
                                        }

                                        form = {
                                            qz_ord: $session.opened_qz_ord,
                                            qz_id: $ad.qzs[$session.opened_qz_ord].id,
                                            mute_state: $ad.qzs[$session.opened_qz_ord].settings.is_muted,
                                            mute_tx: $ad.qzs[$session.opened_qz_ord].settings.is_muted_tx
                                        };
                                        sendAJ("qz_mute", JSON.stringify(form));
                                        break;

                                    case "oqt_save":
                                        let oqt_name = $(".sys_mes .inp_box .inp").val().trim();
                                        oqt_name = oqt_name.replace(/"/g,"\"");
                                        oqt_name = oqt_name.replace(/;/g,"");
                                        oqt_name = oqt_name.replace(/\n/g,"<br>");


                                        if (!oqt_name)
                                            message_ex("show","info","direct","Укажите валидное название шаблона.");
                                        else
                                        {
                                            let comm_gr = duplicate($new_qz.settings.comm_groups[oqt_db.gr_ord]);
                                            delete comm_gr.qb_id;
                                            delete comm_gr.comp_after;
                                            delete comm_gr.qz_after;
                                            delete comm_gr.self_ban_list;
                                            comm_gr.name = oqt_name;
                                            sendAJ("oqt_save", JSON.stringify(comm_gr));
                                        }
                                        break;
                                }
