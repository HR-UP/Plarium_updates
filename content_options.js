...
function feedback_turn_on_block(block) {
    if ("qz_list" === block &&
        !$new_qz.settings.comm_groups[oqt_db.gr_ord].hasOwnProperty("qz_after")
    )
        $(".comm_group .btn_after_qz").trigger("click"); // open qz-list window if isn't
    else
    if ("comp_list" === block &&
        !$new_qz.settings.comm_groups[oqt_db.gr_ord].hasOwnProperty("comp_after")
    )
        $(".comm_group .btn_after_comp").trigger("click"); // open comp-list window if isn't
}
        ...
        .click(function(){
            ...
            switch (action) {
                ...
                case "intros_reset":
                    ...
                    $(".options_letter .intros_wnd .intro").trigger("keyup"); // resave updated texts
                    $session.presets_set();
                    break;
