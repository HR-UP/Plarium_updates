...
function resp_pct_done($d, $qbooks = false)
{
    ...
    {
        if (180 === $d["qz_id"])
            file_put_contents("resp_rec_chk.txt", " \n pct complete: detected by map: step " . $resp["map_step"] . ", len " . $resp["map_len"] . ", eval " . (($resp["map_step"]+1) / $resp["map_len"]), FILE_APPEND);
    ...
    else
    {
        ...
        {
            if (180 === $d["qz_id"])
                file_put_contents("resp_rec_chk.txt", " \n pct complete: disabled by absence of questions", FILE_APPEND);
            $answered = 1;
            $qst_qnt = 1;
        }
    }
    ...
    */
    ...
    foreach ($comm_group["comp_list"] as $comp_id => $qst_list)
        if (is_array($qst_list) && count($qst_list)) // list of questions is not empty
        {
            ...
            foreach ($qst_list as $q_ord => $qst_tx) {
                $found_enabled = true; // true if none cat-specific options are found

                if (isset($comm_group["comp_cats_list"]) &&
                    isset($comm_group["comp_cats_list"][$comp_id]))
                {
                    $found_enabled = false;
    ...
    if ($comm_group["qz_after"]) // there is comment after quiz
    {
        ...
        foreach ($comm_group["qz_list"] as $ord => $qz_comment)
            ...
            {
                $enabled_by_cat = true; // by default, if there is no role-specific enablers, it's on
