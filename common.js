...
function get_resp_fb_done($d)
{
    ...
    if (!$d.resp_ord ) // && qz.settings.self_ban_list[$d.gr_ord]
    {
        // auto-completion if self-eval was disabled // or if it's self-eval at all for that matter
    }
    else
    {
        // Get qbook qsts list
        let $comm_group = qz.settings.comm_groups[$d["gr_ord"]];
