...
function get_resp_fb_done($d)
{
    ...
    if ($resp.hasOwnProperty("feedback") &&
        ...
        obj_len($resp.feedback)
    )
    {
        $resp_feedback_exist = true;
        $fb = $resp.feedback;
    }
    //console.log("user's $fb: ", $fb);
    ...
    if ($comm_group.qz_after) // there is comment after quiz
    {
        for (let i=0; i<$comm_group.qz_list.length; i++)
            if ($comm_group.hasOwnProperty("lock_qz_list") &&
                $comm_group.lock_qz_list.hasOwnProperty(i) &&
                1 === $comm_group.lock_qz_list[i] * 1)
            {
                let $ord = i;
                ...
                if ($enabled_by_cat)
                {
                    ...
                    {
                        //console.log("qz_after: not $resp_feedback_exist");
                        $passed = false;
                        break;
                    }
                    else
                    ...
                    {
                        //console.log("qz_after, answer exist in ord: " + $ord);
                    ...
                    {
                        //console.log("qz_after, no answer slot exist in ord: " + $ord);
                        $passed = false;
                        break;
                    }
                }
    ...
    //console.log("after calculating qz_after: $passed " + $passed);
    //console.log("$present_comp_id_list: ", $present_comp_id_list);
    ...
    {
        //console.log("comp_after exists");
        Object.keys($present_comp_id_list).map(function (key) {
            let $comp_id = key * 1;
            let is_broke = false;
            let $comp = $present_comp_id_list[key];
            ...
            if (!is_broke)
                for (let x=0; x<$comp.length; x++)
                {
                    ...
                    if (!$resp_feedback_exist)
                    {
                        //console.log("comp_after: not $resp_feedback_exist");
                        $passed = false;
                        is_broke = true;
                        break;
                    }
                    ...
                    {
                        //console.log("comp_after, answer exist in ord: " + $comm_ord);
                    ...
                    {
                        //console.log("comp_after, no answer slot exist in ord: " + $comm_ord);
                        $passed = false;
                        is_broke = true;
                        break;
                    }
