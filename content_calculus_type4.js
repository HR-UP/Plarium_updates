//   -------------------------------------------------------------------------------------------------------------------
function report4_indic_by_roles(qz_ord, batch_ord, targ_qz_id) {
    ...
    let head_elems = [
        ...
        "самооценка: балл по компетенции (сумма индикаторов с учетом веса)",

        "оценки руководители",
        "средняя по индикатору умнож на вес",
        "руководители: балл по компетенции (сумма индикаторов с учетом веса)",

        "оценки коллеги",
        "средняя по индикатору умнож на вес",
        "коллеги: балл по компетенции (сумма индикаторов с учетом веса)",

        "оценки подчиненные",
        "средняя по индикатору умнож на вес",
        "подчиненные: балл по компетенции (сумма индикаторов с учетом веса)",

        "оценки смежная команда",
        "средняя по индикатору умнож на вес",
        "смежная команда: балл по компетенции (сумма индикаторов с учетом веса)",

        "Проходной балл компетенции"
    ];
    ...
    */
    ...
    if (cat_col_spans[4])
    {
        head += "<td class='line_comp' colspan='"+ cat_col_spans[4] +"'>"+ head_elems[4] +"</td>"; // BOSS
    ...
    if (cat_col_spans[3])
    {
        head += "<td class='line_comp' colspan='"+ cat_col_spans[3] +"'>"+ head_elems[7] +"</td>"; // COLLEAGUE
    ...
    if (cat_col_spans[5])
    {
        head += "<td class='line_comp' colspan='"+ cat_col_spans[5] +"'>"+ head_elems[10] +"</td>"; // SLAVES
    ...
    if (cat_col_spans[1])
    {
        head += "<td class='line_comp' colspan='"+ cat_col_spans[1] +"'>"+ head_elems[13] +"</td>"; // SLAVES
                    ...
                    if (cat_col_spans[v_cat_id]) // there is resps in that category
                    {
                        for (let q=0; q<q_list.length; q++)
                            if (q_list[q].id === v_qid)
                            {
                                ...
                                if (!resp_list.length)
                                    line += "<td> </td>"; // min 1 empty cell to fill the category gap
                                else
                        ...
                        if (1 === q_index) // !avg_is_placed &&
                        {
                            avg_is_placed = true;
                            line += "<td  style='background-color: #ffeded' rowspan='"+ cat_slot.q_list.length +"'>"+ calculus_colorize_pct(cat_slot.summ, calculus_get_pct(cat_slot.summ / v_comp.targ_avg)) +"</td>";
                        }
                    }
                });

                if (!comp_avg_is_placed)
                {
                    comp_avg_is_placed = true;
                    line += "<td style='background-color: #dbf3ff' rowspan='"+ v_comp.cat_list[0].q_list.length +"'>"+ v_comp.targ_avg +"</td>"; // COMP GATE PTS
                }
    ...
    if (!block_calculus_update) {
        let t = "<table class='calculus' type='4'>" +
...
//   -------------------------------------------------------------------------------------------------------------------
function report4_indic_by_roles_resp(qz_ord, batch_ord, targ_qz_id) {
    let qz = $ad.qzs[qz_ord];
    let group = qz.resps[batch_ord];
    ...
    let head = "<tr>";
    let head_elems = [
        "<b>самооценка:</b> балл по компетенции (сумма индикаторов с учетом веса)",
        "<b>руководители:</b> балл по компетенции (сумма индикаторов с учетом веса)",
        "<b>коллеги:</b> балл по компетенции (сумма индикаторов с учетом веса)",
        "<b>подчиненные:</b> балл по компетенции (сумма индикаторов с учетом веса)",
        "<b>смежная команда:</b> балл по компетенции (сумма индикаторов с учетом веса)",
    ...
    */
    ...
    res.comp_list.forEach(function (v_comp, i_comp) {
        ...
        [0,4,3,5,1].forEach(function (v_cat_id) {
            ...
            {
                let cat_slot = v_comp.cat_list[v_cat_id];
                line += "<td style='background-color: #ffeded;'>"+
                    calculus_colorize_pct(cat_slot.summ, calculus_get_pct(cat_slot.summ / v_comp.targ_avg)) +
                    "</td>";
            }
        });
        line += "<td style='background-color: #dbf3ff;'>"+ v_comp.targ_avg +"</td>"; // COMP GATE PTS
        // INNER INDICATOR LINES END   ------------------------------------------------- + + +
        line += "</tr>";
        body += line;
    });

    if (!block_calculus_update) {
        let t = "<table class='calculus' type='4' resp>" +
            head +
            body +
            "</table>";
        $(".calculus").remove();
