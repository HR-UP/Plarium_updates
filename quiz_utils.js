...
//----------------------------------------------------------------------------------------------------------------------
function get_resp_comments_given_pct(data) {
    ...
    if (comm_group.comp_after)
    {
        given += Object.keys(comm_group.qz_list).length;
    }

    if (comm_group.qz_after)
    {
        Object.keys(comm_group.qz_list).map(function (k) {
    ...
    if (tag.style === "polar_comps")
    {
        ...
        if ([2,6,7,4].indexOf(single_report.calc_type) !== -1)
        {
            ...
            res.comp_list.forEach(function (v_comp) {
                ...
                [0,3,4,5,1].forEach(function (cat_id) {
            ...
            datasets.push({
                label: 'смежная команда',
                backgroundColor: "#a43dfd",//color(window.chartColors.red).alpha(0.5).rgbString(),
                borderColor: "#d272fd",//window.chartColors.red,
                borderWidth: 1,
                data: role_vals[1]
            });
