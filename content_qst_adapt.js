...
function qb_adapt_line_comp(comp_slot, comp_id) {
    ...
    $cats_list_template.forEach(function (cat_id) {
...
function qb_adapt_line_qst(qst_slot, qst_id, comp_id) {
    ...
    let val = "";
    if (null !== qst_slot.targ)
        val = qst_slot.targ;

    s += "<td><div class='qst_targ'><input value='"+ val +"'></div></td>";
    // Cats columns
    let cat_ids = $cats_list_template;
        ...
        if (action === "add_events")
        {
            ...
            $(".qst_targ input")
                ...
                .change(function (e) {
                    // Constraint value to 0-1 (we allow zero only cuz typing dot will floor it down to zero in an instant)
                    let val = $(this).val();
                    let targ_value = qb_adapt_targ_val_constraint(val); // toggle 0-1 values
                    if ("" === val)
                        $(this).val(val);
                    else
                        $(this).val(targ_value);
...
function qb_adapt_bld_qst_structure(qid) {
    ...
    let cat_id_list = $cats_list_template; // self, boss, colleague, employee
    ...
    $(".qbooks_adapt_wnd .btn_box .btn")
        ...
        .click(function(){
            ...
            switch (action) {
                ...
                case "next":
                    let qb_is_shown = $(".qb_adapt_box").css("display");
                    let qb_is_saved = $(".line[type=title] .qb_save").attr("is_saved") * 1;
                    if ("none" !== qb_is_shown &&
                        undefined !== qb_is_saved &&
                        !isNaN(qb_is_saved) &&
                        !qb_is_saved)
                    {
