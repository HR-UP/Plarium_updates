...
//****--------------------------------------------------------------------------------------------------------------****
    function resp($action, $info, $reconnect=true, $no_ans = false)
    {
        ...
        switch ($action)
        {
            ...
            case "get_client_data":
                {
                    ...
                    $d = json_decode($info);
                    $q = $this->send_query("SELECT resps, settings, status, madeby FROM quiz WHERE qkey = '". $d->qkey ."'");
            ...
            case "save_ans":
                {
                    $upd = json_decode($info);
                    $upd->ankete = json_encode($upd->ankete, JSON_UNESCAPED_UNICODE);
                    $upd->ud = time();

                    $q = $this->send_query("UPDATE resp SET ankete = '". $upd->ankete ."', ud = '". $upd->ud ."' WHERE ukey = '".$upd->ukey . "'");
                    ...
                    {
                        log_add("resp->$action: was not saved");
                        echo false;
                    }
...
//>>>>--------->>>>--------->>>>---------->>>>---------->>>>----------->>>>------------>>>>------------>>>>------->>>>--
    function question($action, $info=false)
    {
        $this->DB_conn();
        $owner_id = $_SESSION["pers"]["id"];
