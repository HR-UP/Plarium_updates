...
function calc_empty_cat_columns(sub_empty_cols, group, resp_answers) {
    Object.keys(resp_answers).map(function (key, ind) {
        if (resp_answers[key] === 0)
        {
            //console.log("empty resp id " + key);
            for (let i=0; i<group.length; i++)
                if (group[i].id === key*1 && [3,4,5].indexOf(group[i].cat_id) !== -1)
                {
                    let cat_id = group[i].cat_id;
                    sub_empty_cols[cat_id]++;
                    break;
                }
        }
    });
