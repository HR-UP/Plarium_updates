...
function sendAJ($tag,$data,$tags){
            ...
            .done(function()
            {
                if ($ajResponse[z].readyState === 4 && $ajResponse[z].status === 200)
                {
                    if ($ajResponse[z].responseText)
                    {
                        ...
                        $ad.qzs[qz_ord].status = args.status; // update resps with their new ignore statuses
                        if (-1 === args.status * 1)
                        {
    ...
    if ($tag === "qz_answers_console_hack")
    {
        /*
        sendAJ("qz_answers_console_hack", JSON.stringify({
            qz_id : 181,
            new_ans_list: ["Эксперт","Продвинутый","Базовый","Затрудняюсь ответить"]
        }));
        */
        ...
        // -------------------------------------------------------------------------------------
            .done(function()
            {
                if ($ajResponse[z].readyState === 4 && $ajResponse[z].status === 200)
                {
                    ...
                    if ($ajResponse[z].responseText)
                    {
                        let a = JSON.parse($ajResponse[z].responseText);
                        if (a.hasOwnProperty("error"))
                            message_ex("show", "info", "direct", a.error);
                        else
                            message_ex("show", "info", "direct", "Замена ответов через консоль прошла успешно.");
                    }
                    else
                        message_ex("show", "info", "direct", $tag + " пустой ответ запроса.");
